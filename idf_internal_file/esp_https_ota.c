...
static esp_err_t _ota_write(esp_https_ota_t *https_ota_handle, const void *buffer, size_t buf_len)
{
    ...
    else
    {
        ...
        // ESP_LOGE(TAG, "Written image length %d", https_ota_handle->binary_file_len);
...
esp_err_t esp_https_ota_begin(esp_https_ota_config_t *ota_config, esp_https_ota_handle_t *handle)
{
    ...
    // ESP_LOGI("OTA", "http ota malloc: %d", sizeof(esp_https_ota_t));
    ...
    // ESP_LOGW("OTA", "https_ota_config: partial http download: %d, max_http_request_size: %d", https_ota_handle->partial_http_download, https_ota_handle->max_http_request_size);
    ...
    {
        // ESP_LOGI("OTA", "partial http download");
        ...
        ESP_LOGI("OTA", "Image length: %d", https_ota_handle->image_length);
    ...
    {
        // ESP_LOGI("OTA", "http_client_init_cb");
    ...
    if (!https_ota_handle->partial_http_download)
    {
        // ESP_LOGI("OTA", "no partial_http_download");
        https_ota_handle->image_length = esp_http_client_get_content_length(https_ota_handle->http_client);
        ESP_LOGI("OTA", "image length: %d", https_ota_handle->image_length);
    }
    ...
    *handle = (esp_https_ota_handle_t)https_ota_handle;
    ...
    ESP_LOGI("OTA", "HTTP config: ota upgrade_buf: %d, bulk_flash_erase: %d, binary_file_length: %d", https_ota_handle->ota_upgrade_buf_size, https_ota_handle->bulk_flash_erase, https_ota_handle->binary_file_len);
    return ESP_OK;
...
uint16_t chuck_download_count = 0;
esp_err_t esp_https_ota_perform(esp_https_ota_handle_t https_ota_handle)
{
    ...
    // ESP_LOGI("OTA", "erase_size: %d", erase_size);
    switch (handle->state)
    {
    case ESP_HTTPS_OTA_BEGIN:
        // ESP_LOGW("OTA", "Inside OTA BEGIN LOOP");
        ...
        ESP_LOGI("OTA", "binary file length: %d", binary_file_len);
        // ESP_LOGW("OTA","received buffer length: %d", strlen(handle->ota_upgrade_buf));
        ESP_LOG_BUFFER_HEXDUMP("OTA", handle->ota_upgrade_buf, binary_file_len, ESP_LOG_WARN);
        return _ota_write(handle, (const void *)handle->ota_upgrade_buf, binary_file_len);
    case ESP_HTTPS_OTA_IN_PROGRESS:
        chuck_download_count++;
        data_read = esp_http_client_read(handle->http_client,
                                         handle->ota_upgrade_buf,
                                         handle->ota_upgrade_buf_size);
        // ESP_LOGI("OTA", "data read: %d", data_read);
        if (data_read == 0)
        {
            ESP_LOGI("OTA", "chuck download count: %d", chuck_download_count);
            /*
        ...
        {
            // ESP_LOGI("OTA", "ota write operation performed");
        ...
        {
            ESP_LOGI("OTA", "ESP_HTTP_OTA_SUCCESS");
...
esp_err_t ota_through_simcom(esp_https_ota_handle_t *handle, int binary_length)
{
    esp_err_t err;
    ...
    https_ota_handle->partial_http_download = 0;
    https_ota_handle->max_http_request_size = 65536;
    https_ota_handle->image_length = binary_length;
    ...
    //++ allocate memory for ota_upgrade_buf
    const int alloc_size = 289;
    https_ota_handle->ota_upgrade_buf = (char *)malloc(alloc_size);
    if (!https_ota_handle->ota_upgrade_buf)
    {
        ESP_LOGE(TAG, "Couldn't allocate memory to upgrade data buffer");
    ...
    https_ota_handle->ota_upgrade_buf_size = alloc_size;
    https_ota_handle->bulk_flash_erase = 0;
    https_ota_handle->binary_file_len = 0;
    *handle = (esp_https_ota_handle_t)https_ota_handle;
    https_ota_handle->state = ESP_HTTPS_OTA_BEGIN;
...
extern char ota_bin_chuck_data[11560];
extern int bin_chuck_data_length;
extern bool http_data_read;
extern int bin_size;
bool complete_bin_received = false;

esp_err_t esp_simcom_ota_perform(esp_https_ota_handle_t https_ota_handle)
{
    esp_https_ota_t *handle = (esp_https_ota_t *)https_ota_handle;
    if (handle == NULL)
    {
    ...
    case ESP_HTTPS_OTA_BEGIN:
        ...
        {
            int command_len = strlen("AT+HTTPREAD=0,5780\r\n");
            data_read = uart_write_bytes(UART_NUM_1, "AT+HTTPREAD=0,5780\r\n", command_len);
            ESP_LOGW("WRITE", "--[ wrote %d bytes]--", data_read);

            while (1)
            {
                if (http_data_read == true)
                {
                    http_data_read = false;
                    break;
                }
                vTaskDelay(50 / portTICK_PERIOD_MS);
            }

            binary_file_len = bin_chuck_data_length;
            handle->ota_upgrade_buf = &ota_bin_chuck_data;
            // ESP_LOG_BUFFER_HEXDUMP("OTA", handle->ota_upgrade_buf, 578, ESP_LOG_WARN);
        }
        err = esp_ota_verify_chip_id(handle->ota_upgrade_buf);
        if (err != ESP_OK)
        {
    ...
    case ESP_HTTPS_OTA_IN_PROGRESS:
    {
        ESP_LOGI("OTA", "binary file length: %d, bin size: %d", handle->binary_file_len, bin_size);

        if (handle->binary_file_len == bin_size)
        {
            complete_bin_received = true;
            ESP_LOGI("OTA", "complete bin received");
            bin_chuck_data_length = 0;
        }

        if (complete_bin_received == false)
        {
            int len = strlen("AT+HTTPREAD=0,5780\r\n");
            uart_write_bytes(UART_NUM_1, "AT+HTTPREAD=0,5780\r\n", len);
            ESP_LOGW("WRITE", "--[ wrote %d bytes]--", len);
        ...
        handle->ota_upgrade_buf = &ota_bin_chuck_data;
        data_read = bin_chuck_data_length;
